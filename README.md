...
| 🧠 **Learnings system** | Accumulated incident knowledge organized by category (rds, redis, drainer…). Agent reads relevant categories at investigation start via `learnings_list` + `learnings_read` tools. Teach it new facts via `@sage learn <category> <fact>`. |
...
The bot ships with the **Sage** persona — an SRE character used at ExampleApp. To use your own bot name and identity, edit `bot/slack.py` → `_simple_chat()` system prompt:

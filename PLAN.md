...
>
> - **Branch:** `argus` work now lives on `main`; AWS prod runs legacy v1.1.23, GCP Argus runs **1.0.25-argus** (2026-08-07 speed+accuracy release: no-thinking tool steps [VK_THINK_ON_TOOL_STEPS], parallel runbook batching, lean runbook prompts, verify at step 8/6 [VK_VERIFY_AFTER/EVERY], 12k compression threshold [VK_MAX_TOOL_OUTPUT], deduped runbook retrieval, throttled async Slack status, parallel pattern replay, 30s sub-agent cap [VK_SUB_AGENT_TIMEOUT], PDF upload retry + channel-ID harvest from postMessage [no channels:read scope needed], reaper-path PDFs. Test RCA: ~5.5 min vs ~10). **DB topology verified + corrected everywhere** (knowledge base + all 15 runbooks, now with parallel execution-plan waves): app DB = AlloyDB rider-db-cluster/driver-db-cluster; Aurora app clusters DELETED (only atlas-utils-db-cluster remains); bap_pg=10.60.102.55/atlas_app_pilot, bpp_pg=10.60.102.193/atlas_driver_offer_bpp (non-critical reader). LLM: gateway key sk-AVDu… (10 parallel), model open-large-sa (GLM 5.2), fast open-fast-sa (MiniMax M3) — kimi keys dead/blocked.
...
> - **ARCHITECTURE (2026-08-08, user-locked): TWO INDEPENDENT ALL-IN-ONE INSTANCES — no orchestrator/executor cross-cloud routing.**
>   Same image; each deployment is locked to its cloud via env: `VK_CLOUD=gcp` + `VK_DEFAULT_CLOUD=gcp` (GCP) / `VK_CLOUD=aws` + `VK_DEFAULT_CLOUD=aws` (AWS). The existing per-cloud hard filter (server.py cloud_router) drops other-cloud alerts; runbook retrieval facets by `issue.labels.cloud or config.cloud`; knowledge = per-cloud `site_knowledge` rows; each deployment carries ONLY its own Slack bot tokens (GCP=Argus, AWS=Sage). Cross-cloud facts appear in runbooks only as context notes ("AWS apps also write to the shared AlloyDB — not inspectable from here; flag as UNVERIFIED"), never as executable steps.
>   - **GCP instance: LIVE** (1.0.25-argus → 1.0.26; ns `argus`, gke_ny-prod). GCP-only runbooks (cloud_type=gcp), AWS steps stripped; verified the pod has NO AWS creds/context/ES reach.
>   - **AWS instance: PENDING** — replace legacy v1.1.23 (`monitoring/vishwakarma`, ECR) with this image: push to ECR, VK_CLOUD=aws env, Sage tokens only, author cloud_type=aws runbooks from the pre-strip backups (/Users/vijaygupta/misc/tmp/infra-backups/runbooks-20260807-202423 + scratchpad optimized pre-strip versions), write `site_knowledge` cloud='aws' row (ElastiCache/ALB/EKS/utils-RDS endpoints), IRSA for CloudWatch/EKS read.
> - Pending externals: GitHub App token; golden-set labeling for `vk eval`.
>

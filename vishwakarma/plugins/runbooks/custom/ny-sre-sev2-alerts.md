...
## Goal
- **Primary Objective:** Investigate SEV2-level alerts — Protocol 5xx errors, external gateway failures (Acme, OpenNetwork), ride-to-search ratio drops, and allocator issues.
...
### Alert: Protocol5xxErrors / ProtocolExternalAPI5xx / ProtocolIncomingAPI5xx
**Trigger:** Protocol API returning 5xx errors above threshold.
...
6. Check if an external provider caused the 5xx:
   `timeout 30 stern -n atlas <service-name> --since 30m | grep -iE "acme|opennetwork|google|external|downstream|outbound|provider" | head -50`
...
### Alert: AcmeGateway5xx / AcmeRegistry5xx
**Trigger:** Acme gateway or registry returning 5xx errors.

1. Find which service is making Acme calls and getting 5xx:
   `sum(increase(http_request_duration_seconds_count{status_code=~"5[0-9]{2}"}[5m])) by (service, handler)`
   Look for handlers with "acme", "payment", "gateway" in the name.

2. Find that service's pods: `kubectl get pods -A | grep -i <service-name>`

3. Grep logs for Acme errors:
   `timeout 30 stern -n atlas <service-name> --since 30m | grep -iE "acme|payment|gateway|5[0-9]{2}|error|timeout" | head -200`

4. Search Elasticsearch `app-logs-<today's date>` for Acme-related errors in the last 30 minutes:
```json
{
  "size": 20,
  "query": {
...
6. Determine: is Acme returning 5xx (external outage) or is SRE Platform sending bad requests (request format issue)?
   Look for HTTP response body in logs — a 5xx with Acme's error message = external issue.

**Possible root causes:** Acme outage, expired API credentials, bad request format after code change.

---

### Alert: OpenNetworkGateway5xx / OpenNetworkRegistry5xx
**Trigger:** OpenNetwork gateway or registry returning 5xx errors.

1. Find which service is making OpenNetwork calls:
   `sum(increase(http_request_duration_seconds_count{status_code=~"5[0-9]{2}"}[5m])) by (service, handler)`

2. Find that service's pods: `kubectl get pods -A | grep -i <service-name>`

3. Grep logs for OpenNetwork errors:
   `timeout 30 stern -n atlas <service-name> --since 30m | grep -iE "opennetwork|registry|gateway|5[0-9]{2}|error|timeout" | head -200`

**Possible root causes:** OpenNetwork outage, API version mismatch, bad credentials, network issue.
...
### If logs show external provider errors (acme, opennetwork, timeout to external URL)

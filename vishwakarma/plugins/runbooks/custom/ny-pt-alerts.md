...
```
kubectl get pods -n atlas | grep -iE "public-transport|pt-backend|protocol-app"
...
```
kubectl rollout history deployment -n atlas | grep -i "payment\|refund\|acme"
kubectl get events -n atlas | grep -iE "deploy\|rollout" | tail -20
```

### Step 3: Check Acme Gateway Health (see ny-sre-sev2 runbook if AcmeGateway5xx also firing)

### Synthesis
- **Acme errors in logs + refund spike** → Acme gateway issue. Monitor; check Acme status.
...
- **Brief spike, resolves** → Likely transient Acme timeout. Safe to watch unless persists >10 min.

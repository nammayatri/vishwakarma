...
Alert names do not always map 1:1 to instance names (e.g. `app-db-r1` → `bap-reader-1`).
...
- query: `rate(protocol_ride_created_total[5m]) / rate(protocol_search_request_total[5m])`
- start: `<startsAt - 30m>`
- end: `<startsAt + 1h>`
- step: `5m`

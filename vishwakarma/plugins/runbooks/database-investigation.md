...
```sql
-- Rider
SELECT id, first_name, created_at FROM app_db.person WHERE mobile_number_hash = sha256('<phone>') LIMIT 5
-- Driver
SELECT id, first_name, created_at FROM provider_db.person WHERE mobile_number_hash = sha256('<phone>') LIMIT 5
```

### If given a ride short ID
```sql
SELECT * FROM provider_db.ride WHERE short_id = '<short_id>' LIMIT 1
```
...
5. **Payment** — `SELECT * FROM app_db.payment_order WHERE order_id = '<bap_booking_id>' LIMIT 5`
6. **Fare details** — `SELECT * FROM provider_db.fare_parameters WHERE id = '<fare_parameters_id>' LIMIT 1`
7. **Driver** — `SELECT id, first_name, active, enabled, mode FROM provider_db.person WHERE id = '<driver_id>' LIMIT 1`
8. **Rider** — `SELECT id, first_name FROM app_db.person WHERE id = '<rider_id>' LIMIT 1`
...
SELECT id, status, fare, created_at FROM provider_db.ride
...
SELECT cancellation_reason, count(*) FROM provider_db.booking_cancellation_reason
WHERE created_at >= '<1h_ago>' GROUP BY cancellation_reason ORDER BY count DESC LIMIT 10

-- Payment failures by gateway
SELECT status, count(*) FROM app_db.payment_order
WHERE created_at >= '<1h_ago>' GROUP BY status ORDER BY count DESC
```
